...
class MemoryManager {
    ...
    async storeInteraction(participants, context, events, emotions, outcomes) {
    ...
    calculateSignificance(events, emotions, outcomes) {
        ...
        const hasLearning = learningKeywords.some(keyword => outcomes.toLowerCase().includes(keyword) ||
    ...
    async searchMemories(query) {
        const memories = await this.db.queryEpisodicMemories(query.timeRange, query.context, query.significance_threshold, query.limit);
    ...
    getMatchReasons(memory, query) {
        const reasons = [];
        if (query.context && memory.context === query.context) {
            reasons.push(`Context match: ${query.context}`);
        }
        if (query.participants) {
            const commonParticipants = memory.participants.filter(p => query.participants.includes(p));
    ...
    async scheduleConsolidation(memoryId) {
    ...
    hasThematicOverlap(memory1, memory2) {
        // Check for participant overlap
        const participantOverlap = memory1.participants.some(p => memory2.participants.includes(p));
        // Check for keyword overlap in events/outcomes
        const getAllWords = (mem) => [...mem.events, mem.outcomes].join(' ').toLowerCase().split(/\W+/);
